...
int i, j, guard_id, winning_guard, best_minute, best_minute_details;
char action[256] = {};
  ...
  for(i = 1; i<INPUT_SIZE; i++) {
    ...
    if(input[i].operation == wake_up) {
      ...
      for(j = input[i-1].minute ; j < input[i].minute ; j++) {
        ...
        if(guard_sleep_minutes[input[i].guard_id][j] > best_minute_details) {
          winning_guard = input[i].guard_id;
          best_minute = j;
          best_minute_details = guard_sleep_minutes[input[i].guard_id][j];
        }
      }
    }
  }
